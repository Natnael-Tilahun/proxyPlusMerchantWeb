...
const {getMerchantOperators, getMerchantOperatorsByBranchId} = useOperators()
...
const fetchMerchantOperatorsData = async () => {
  try {
    ...
    const merchantOperators = await getMerchantOperatorsByBranchId(branchId.value);
...
<template>
  ...
  <div v-if="isError || data == null || data == undefined">
