...
import { inviteNewMerchantOperatorFormSchema } from "~/validations/inviteNewMerchantOperatorFormSchema";
import { ref } from "vue";
...
const fetchMerchantsData = async () => {
  try {
    isError.value = false;
    loading.value = true;
    const response = await getBranches(0, 100000);
    branchesData.value = response?.filter(branch => branch.merchantBranchId == myBranchId.value);
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            <FormField v-slot="{ componentField }" name="phoneNumber">
              <FormItem class="w-full">
                ...
                <FormControl>
                  <UiInput type="text" placeholder="Enter operator phone number" v-bind="componentField" />
                </FormControl>
                <FormMessage />
              </FormItem>
            </FormField>
