...
import { columns as tableColumns } from "~/components/branches/mine/columns"; // Renamed to avoid conflict
...
const fetchMerchantBranchesData = async () => {
  try {
    ...
    const merchantBranches = await getBranches(0, 10000);
    data.value = merchantBranches?.filter(
      (branch) => branch.merchantBranchId == myBranchId.value
    );
    console.log("data: ", data.value);
    // Sort integrations by name alphabetically
    data.value =
      data.value?.sort((a: Branch, b: Branch) => {
