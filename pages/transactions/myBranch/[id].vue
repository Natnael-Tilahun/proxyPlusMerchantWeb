...
const { getBranchTransactionsById } = useTransactions({
  autoFetch: false,
});
...
try {
  ...
  transactionData.value = await getBranchTransactionsById(myBranchId.value, id);
  transactionId.value = transactionData.value?.merchantTransactionId;
  if (
    transactionData.value?.paymentStatus == "PENDING" &&
    transactionData.value?.merchantTransactionId
  ) {
    connect(transactionId.value || "");
  }
} catch (error) {
  console.error("Error fetching branch transaction details:", error);
} finally {
...
<template>
  <div class="w-full flex flex-col gap-8">
    ...
    <UiCard
      ...
      class="p-6 grid lg:grid-cols-5 gap-8 w-full"
    >
      <div class="w-full lg:col-span-2">
        <DashboardTransactionDetail :transactionDetails="transactionData" />
      </div>
      <div
        class="grid grid-cols-1 md:grid-cols-2 md:col-span-2 lg:col-span-3 gap-6 w-full"
      >
        ...
        />
        <TransactionsTransactionDetailItem
          label="Transaction Completed Date"
        ...
        <TransactionsTransactionDetailItem
          ...
          :value="
            transactionData.merchantAccountNumber &&
            formatAccountNumber(
              typeof transactionData.merchantAccountNumber === 'string'
                ? transactionData.merchantAccountNumber
                : '-'
            )
          "
        ...
        <TransactionsTransactionDetailItem
          ...
          :value="formatAccountNumber(transactionData.payerAccountNumber)"
        />
        <TransactionsTransactionDetailItem
          label="Payer Id"
  ...
  >
    <UiDialogContent
      class="max-w-md w-full rounded-xl overflow-y-scroll shadow-lg bg-white dark:bg-zinc-900 p-0"
    >
      <div class="flex flex-col items-center py-8 px-6">
        ...
        <!-- Title -->
        <UiDialogTitle
          class="text-2xl font-bold text-green-700 mb-2 text-center"
        >
        ...
        <!-- Summary -->
        <UiDialogDescription
          class="text-base text-center text-zinc-700 dark:text-zinc-200 mb-4"
        >
          ...
          <span>
            ({{ transactionData?.currencyCode }}-{{
              transactionData?.payerAccountNumber?.slice(-4)
            }}) for
          </span>
          ...
          <span>
            ({{ transactionData?.currencyCode }}-{{
              transactionData?.merchantAccountNumber?.slice(-4)
            }})</span
          >
        </UiDialogDescription>
        <!-- Details -->
        <div
          class="w-full text-sm text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 mb-4"
        >
          ...
          <div class="flex justify-between mb-2">
            ...
            <span>{{
              formatDate(
                transactionData?.time || transactionData?.completedDate
              )
            }}</span>
          </div>
          <div class="flex justify-between mb-2">
          ...
          <div class="flex justify-between mb-2">
            ...
            <span
              >{{ transactionData?.currencyCode }}
              {{ transactionData?.tipAmount }}</span
            >
          </div>
